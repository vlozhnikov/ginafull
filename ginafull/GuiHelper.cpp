...
bool GuiHelper::ExtractComboboxText(HWND hDlg, int id, wchar_t** ppText)
{
	HWND hwnd = GetDlgItem(hDlg, id);
	if (!hwnd) return false;
	...
	*ppText = new wchar_t[cchMax];
	memset(*ppText, 0, sizeof(wchar_t) * 256);
    if (!*ppText)
	{
        LOOM;
        return false;
	...
	SendMessage(hwnd, CB_GETLBTEXT, curSel, (LPARAM)item);

	swscanf(item, L"%[^(this computer)]", *ppText);
