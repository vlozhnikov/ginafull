                       ...
                       PHANDLE phToken, PWLX_MPR_NOTIFY_INFO pNprNotifyInfo, PVOID* ppWinLogonProfile)
{
    ...
    *pdwOptions = 0; // we always let WinLogon load the user profile for us
	...
	if (isConnected)
	{
		if (!Commander::SendSetProtocolCommand(packetTo, client, NULL) ||
			!Commander::SendIdentityCommand(packetTo, client, NULL) ||
			!Commander::SendStartIdentificationCommand(packetTo, client, NULL))
		{
			_pWinLogon->wlxMessageBox(0,
			L"Persis in to Server is not available. You can not logon to system.",
			L"Security",
			MB_ICONERROR | MB_OK);
			return WLX_SAS_ACTION_NONE;
		}
	}
	else if (Commander::IsServerConnected1())
	{
		if (_pWinLogon->wlxMessageBox(0,
    ...
    if (SecurityHelper::GetLogonSid(*phToken, pLogonSid, LOGON_SID_SIZE))
	{
		...
		if (isConnected &&
			!Commander::SendPersonAuthentificationCommand(packetTo, client, pLogonSid, (const char*)_bstr_t(userName)))
		{
			success = false;
		}
	...
	{
		if (isConnected &&
			!Commander::SendTerminalPersonAuthorizationCommand(packetTo, client, pLogonSid, (const char*)_bstr_t(userName)))
		{
			_pWinLogon->wlxMessageBox(0,
			L"Persis in to Server is not available. You can not logon to system.",
			L"Security",
		...
		else if (!isConnected && Commander::IsServerConnected2())
		{
			if (_pWinLogon->wlxMessageBox(0,
...
{
	//if (_processed) DisplayLockedNotice();
...
int Gina::WkstaLockedSAS(DWORD dwSasType)
{
    ...
    if (WLX_SAS_TYPE_CTRL_ALT_DEL == dwSasType)
	{
        ...
        if (IDOK == dlg.Show())
		{
			...
			{
				BYTE pLogonSid[LOGON_SID_SIZE] = {0};
				if (SecurityHelper::GetLogonSid(hUnlockToken, pLogonSid, LOGON_SID_SIZE))
				{
					if (isConnected &&
						!Commander::SendPersonAuthentificationCommand(packetTo, client, pLogonSid, (const char*)_bstr_t(dlg.userName)))
					{
					}
					if (isConnected &&
						!Commander::SendTerminalPersonAuthorizationCommand(packetTo, client, pLogonSid, (const char*)_bstr_t(dlg.userName)))
					{
						_pWinLogon->wlxMessageBox(0,
							L"Persis in to Server is not available. You can not logon to system.",
							L"Security",
            ...
            else
			{
                ...
                /*wchar_t msg[256];
                Log::LookupErrorMessage(msg, sizeof msg / sizeof *msg, win32Error);
                _pWinLogon->wlxMessageBox(0, msg, L"Logon Message", MB_SYSTEMMODAL | MB_ICONHAND);
				_processed = false;*/
            }
        }
		/*else
